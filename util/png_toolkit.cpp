                              ...
                              Error &err, int &diffPix, filter::base::area const &ar ) const
{
    ...
    for (int y = ar.top == 0 ? 0 : imgData.h / ar.top; y < imgData.h / ar.bottom; y++)
        for (int x = ar.left == 0 ? 0 : imgData.w / ar.left; x < imgData.w / ar.right; x++)
    {
        norm = euclNorm2(sub(     imgData.pixels + (y * imgData.w + x) * cpp,
                             tool.imgData.pixels + (y * imgData.w + x) * cpp));
        diffPix += norm != 0;
